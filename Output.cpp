...
void Output::DrawTriangle(int triangleCenterX, int triangleCenterY, int triangleHeight, int triangleWidth, Direction direction, color triangleColor, drawstyle style, int penWidth) const
{
	...
	if (triangleHeight <= 0 || triangleWidth <= 0 || triangleHeight > UI.CellHeight || triangleWidth > UI.CellWidth)   //validation
		return;
	...
	pWind->SetPen(triangleColor, penWidth);  // outline
	pWind->SetBrush(triangleColor);                   // color filled in done
...
void Output::DrawImageInCell(const CellPosition& cellPos, string image, int width, int height) const
{
	...
	int dX = (UI.CellWidth - width) / 2;
	int dY = (UI.CellHeight - height) / 2;
	if (dX < x)
		dX = x;
	if (dY < y)
		dY = y;
	if (dX + width > x + UI.CellWidth)
		dX = x + UI.CellWidth - width;
	if (dY + height > y + UI.CellHeight)
		dY = y + UI.CellHeight - height;
	pWind->DrawImage(image, dX, dY, width, height);
...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_SET_ANTENNA_CELL] = "images\\Menu_SetAntenna.jpg";
	MenuItemImages[ITM_SET_BELT] = "images\\Menu_SetBelt.jpg";
	MenuItemImages[ITM_SET_WATER_PIT] = "images\\Menu_SetWaterPit.jpg";
	MenuItemImages[ITM_SET_DANGER_ZONE] = "images\\Menu_SetDangerZone.jpg";
	MenuItemImages[ITM_SET_WORKSHOP] = "images\\Menu_SetWorkshop.jpg";
	MenuItemImages[ITM_SET_ROTATING_GEAR] = "images\\Menu_SetRotatingGear.jpg";
	MenuItemImages[ITM_COPY_GAME_OBJECT] = "images\\Menu_Copy.jpg";
	MenuItemImages[ITM_CUT_GAME_OBJECT] = "images\\Menu_Cut.jpg";
	MenuItemImages[ITM_PASTE_GAME_OBJECT] = "images\\Menu_Paste.jpg";
...
>>>>>>> ff974dcd19dcd7ff660b336f047b9c3618744a2a
...
void Output::CreateCommandsBar(Command savedCommands[], int savedCommandsCount, Command availableCommands[], int availableCommandsCount) const
{
	...
	CommandItemImages[ROTATE_COUNTERCLOCKWISE] = "images\\RotateClockwiseCard.jpg";
...
void Output::DrawPlayer(const CellPosition& cellPos, int playerNum, color playerColor, Direction direction) const
{
	...
	if (!cellPos.IsValidCell() || playerNum < 0 || playerNum >= MaxPlayerCount)    // keda only player 0 and 1 as MaxPlayerCount is 2 
		return;
...
// TODO: Draw the player triangle in center(x,y) and filled with the playerColor passed to the function
	DrawTriangle(x, y, radius * 2, radius * 2, direction, playerColor, FILLED, 1);
}
...
void Output::DrawBelt(const CellPosition& fromCellPos, const CellPosition& toCellPos) const
{
	...
	int beltFromCellY = fromCellStartY + UI.CellWidth / 2;
	int beltToCellY = toCellStartY + UI.CellWidth / 2;
...
void Output::DrawFlag(const CellPosition& cellPos) const
{
	...
	// TODO: 1. Draw the flag pole (the line)==============DONE
	int flagPoleStartX = cellStartX + UI.CellWidth / 2;
	int flagPoleStartY = cellStartY + UI.CellHeight / 4;

	int flagPoleEndY = flagPoleStartY + UI.FlagPoleHeight;     // y the upper part of the pole
	int flagPoleEndX = flagPoleStartX;

	pWind->SetPen(UI.FlagPoleColor, 2);
	pWind->DrawLine(flagPoleStartX, flagPoleStartY, flagPoleEndX, flagPoleEndY);

	// 		 2. Draw the flag (the triangle)
	DrawTriangle((flagPoleEndX + UI.FlagWidth / 2), (flagPoleStartY + UI.FlagHeight / 2), UI.FlagHeight, UI.FlagWidth, RIGHT, UI.FlagColor, FILLED, 1);
...
void Output::DrawRotatingGear(const CellPosition& cellPos, bool clockwise) const
{
	...
	int RotateGearWidth = UI.CellWidth / 2;
	int RotateGearHeight = UI.CellHeight / 2;
	string gearImage;
	if (clockwise)
	{
		gearImage = "images\\ClockwiseGear.jpg";

	}
	else
	{
		gearImage = "images\\CounterClockwiseGear.jpg";

	}


	DrawImageInCell(cellPos, gearImage, RotateGearWidth, RotateGearHeight);
...
void Output::DrawAntenna(const CellPosition& cellPos) const
{
	...
	string antennaImagePath = "Images\\Antenna.jpg";
	int antennaWidth = UI.CellWidth / 2;
	int antennaHeight = UI.CellHeight / 2;
	DrawImageInCell(cellPos, antennaImagePath, antennaWidth, antennaHeight);
...
void Output::DrawWorkshop(const CellPosition& cellPos) const
{
	...
	string WorkshopImagePath = "Images\\Workshop.jpg";
	int WorkshopWidth = UI.CellWidth / 2;
	int WorkshopHeight = UI.CellHeight / 2;
	DrawImageInCell(cellPos, WorkshopImagePath, WorkshopWidth, WorkshopHeight);
}
